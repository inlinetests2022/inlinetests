...
public class Stream13 {
    ...
    private CalculatedQueryOperation unwrapFromAlias(CallExpression call) {
        ...
        List<String> aliases = children.subList(1, children.size()).stream()
                ...
                .collect(toList());
        new Here().given(children, 
            Arrays.asList(new Expression[] {new SqlCallExpression("SELECT MIN(Price) AS SmallestPrice FROM Products; "), 
                          new SqlCallExpression("SELECT COUNT(ProductID) FROM Products;") }))
            .checkEq(aliases, Arrays.asList("SELECT COUNT(ProductID) FROM Products;"));
