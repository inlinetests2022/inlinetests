...
public class Regex16 {
    ...
    private static long getContentLength(HttpURLConnection connection) {
        ...
        if (!TextUtils.isEmpty(contentRangeHeader)) {
            ...
            new Here().given(CONTENT_RANGE_HEADER, Pattern.compile("^bytes (\\d+)-(\\d+)/(\\d+)$")).given(contentRangeHeader, "bytes 12-34/56")
                .checkTrue(matcher.find()).checkEq(matcher.group(1), "12").checkEq(matcher.group(2), "34").checkEq(matcher.group(3), "56");
            if (matcher.find()) {
                try {
