...
class Regex3 {
    ...
    public void getFlatFields(
        ...
        new Here().given(fieldExpression, "a.b")
            .given(PATTERN_NESTED_FIELDS_WILDCARD, Pattern.compile("([\\p{L}_\\$][\\p{L}\\p{Digit}_\\$]*|[0-9]+)(\\.(.+))?|\\*|\\_"))
            .checkTrue(matcher.find()).checkEq(matcher.group(0), "a.b");
        if (!matcher.matches()) {
